...
hvi_transform = RGB_HVI().to(device)
from model.dino_loss import DinoV2Loss 
import warnings
warnings.filterwarnings("ignore")

os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
import torch
print("CUDA devices detected:", torch.cuda.device_count())
...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        if opt.gamma:
            ...
            output_rgb, output_hvi = model(im1 ** gamma)  
        else:
            output_rgb, output_hvi = model(im1)  
            
        gt_rgb = im2
        #print(output_rgb)
        #output_hvi = hvi_transform.HVIT(output_rgb) 
        #print(output_hvi)
        gt_hvi = hvi_transform.HVIT(gt_rgb) 
        #print(gt_hvi)
        ...
        loss_dino = dino_loss_fn(output_rgb, gt_rgb)
        # 打印每一项损失
        #print(f"L1 Loss (HVI): {L1_loss(output_hvi, gt_hvi).item()}")
        #print(f"D Loss (HVI): {D_loss(output_hvi, gt_hvi).item()}")
        #print(f"E Loss (HVI): {E_loss(output_hvi, gt_hvi).item()}")
        #print(f"P Loss (HVI): {P_loss(output_hvi, gt_hvi)[0].item()}")

        #print(f"L1 Loss (RGB): {L1_loss(output_rgb, gt_rgb).item()}")
        #print(f"D Loss (RGB): {D_loss(output_rgb, gt_rgb).item()}")
        #print(f"E Loss (RGB): {E_loss(output_rgb, gt_rgb).item()}")
        #print(f"P Loss (RGB): {P_loss(output_rgb, gt_rgb)[0].item()}")
        
        loss = loss_rgb + opt.HVI_weight * loss_hvi + opt.dino_weight * loss_dino 
        iter += 1
...
def init_loss():
    ...
    dino_loss_fn = DinoV2Loss(vit_pretrain_path=opt.vit_path).to(device)
    return L1_loss,P_loss,E_loss,D_loss, dino_loss_fn
...
if __name__ == '__main__':  
    ...
    L1_loss,P_loss,E_loss,D_loss, dino_loss_fn= init_loss()
