...
eval_parser.add_argument('--unpaired_weights', type=str, default='./weights/LOLv2_syn/epoch_1500.pth')

ep = eval_parser.parse_args()


def eval(model, testing_data_loader, model_path, output_folder,norm_size=True,LOL=False,v2=False,unpaired=False,alpha=1,gamma=1.0):
    ...
    print('Evaluation:')
    if LOL:
        model.module.trans.gated = True
    elif v2:
        model.module.trans.gated2 = True
        model.module.trans.alpha = alpha
    elif unpaired:
        model.module.trans.gated2 = True
        model.module.trans.alpha = alpha
    for batch in tqdm(testing_data_loader):
        with torch.no_grad():
            ...
            input = input.cuda()
            output_rgb, output_hvi = model(input**gamma)
            
        if not os.path.exists(output_folder):          
            os.mkdir(output_folder)  
            
        output = torch.clamp(output_rgb.cuda(),0,1).cuda()
        if not norm_size:
            output = output[:, :, :h, :w]
    ...
    if LOL:
        model.module.trans.gated = False

    elif v2:
        model.module.trans.gated2 = False
...
if __name__ == '__main__':
    ...
    if ep.lol:
        ...
        if ep.perc:
            weight_path = './weights/LOLV1/epoch_640.pth'
        else:
        ...
        if ep.perc:
            weight_path = './weights/Synthetic_train/epoch_420.pth'
        else:
